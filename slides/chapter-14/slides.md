...
![程序框架](./assets/figures/pv-framework.png){fit="contain" position="center" max-height="44vh"}

- 目标：用 Storm 拓扑计算**京东网站**的页面浏览量，用到 Storm 和 HBase 技术
- 流程：**Kafka**（数据源）→ **PVSpout**（读取）→ **PVBolt**（分 task 统计）→ **PVSumBolt**（汇总）→ **HBase / mysql**（存储）
