...
<!--
**[带读]** 专用过滤器是"开箱即用"的套路，各解决一类典型需求。挑几个常用的说：SingleColumnValueFilter 根据某个参考列的值来决定要不要这一行，是实现条件筛选的利器；PrefixFilter 按行键前缀过滤，相当于按前缀"切片"；PageFilter 做行分页，配合循环就能实现翻页扫描；KeyOnlyFilter 只取键不取值，用来"只看结构不看内容"效率极高。如果你需要"一行的第一列"FirstKeyOnlyFilter、按时间戳控制版本 TimestampsFilter 也都有。这些专用过滤器，让你不必每次都手写复杂的比较逻辑。
