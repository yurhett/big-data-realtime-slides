...
- 还有一个属性——<strong>有序（sequential）</strong>节点：创建时在路径结尾追加递增计数（`%10d` 格式，如 0000000001），溢出时超过 2³²-1
...
- 采用<strong>基于通知的机制</strong>：客户端对 znode 设置<strong>监视点（watch）</strong>接收通知
...
<strong>Watch（监视点）</strong>就像"设一个警报铃"：
