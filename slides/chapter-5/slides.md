...
- 许多协作原语（如分布式锁）常被共享，暴露 create/acquire/release 三个 API；但这种方式**灵活性差**
- Zookeeper **不直接暴露原语**，而是暴露由一小部分调用方法组成的**类文件系统 API**，让应用自己实现原语
- 这些原语的实现称为**菜谱（recipes）**；基本单位是小数据节点——**znode**
- 数据模型采用类似文件系统的**层级树状结构**

![层级树状结构](./assets/figures/znode-tree.png){fit="contain" position="center" max-height="34vh"}
...
- 两种运行模式：
  - **独立模式（standalone）**：单个服务器，状态无法复制
  - **仲裁模式（quorum）**：一组服务器组成 **Zookeeper 集合**，状态复制、同时响应客户端
- 本质上是一个**分布式的小文件存储系统**（原 Hadoop 组件，后拆为独立子项目）

![Zookeeper 架构总览](./assets/figures/zookeeper-architecture.png){fit="contain" position="center" max-height="34vh"}
...
![Zookeeper 处理客户端请求流程](./assets/figures/request-flow.png){fit="contain" position="center" max-height="52vh"}
